...
export class SceneController {
  ...
  start(): void {
    ...
    const loop = (now: number) => {
      ...
      // Clamped at both ends. The ceiling stops a backgrounded tab from
      // resuming with one enormous step; the floor matters because a frame
      // timestamp is not guaranteed to be later than the wall clock reading
      // that started the loop, and a negative dt turns every damped follow into
      // a divergent one.
      const dt = Math.min(Math.max((now - this.lastTime) / 1000, 0), 0.1);
      this.lastTime = now;
      this.tick(dt);
    };
    this.frameHandle = requestAnimationFrame(loop);
